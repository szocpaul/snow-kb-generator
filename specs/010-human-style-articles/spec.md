...
- **FR-004**: A GEPA futás időkerete kontrollált: `max_metric_calls = 200` (időkorlát, nem kvótavédelem — a rolloutok lokálisak; a judge-hívásokkal együtt ~1.5-2 óra a `-np 4` slotokkal — 2026-08-08: a szerver `-np 2`-ről `-np 4`-re bővült, preflight-tal verifikálva). A reflection/proposer hívások továbbra is Kimi K3-ra mennek, azok száma kicsi. A `num_threads` a szerver slotjaihoz igazított (4).
